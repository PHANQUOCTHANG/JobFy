...
import { Loader2, CheckCircle2, AlertCircle, FileText, Send, Upload, Edit3, ShieldCheck, MapPin, AlignLeft, Save, X } from "lucide-react";
...
const EmployerSettingsPage = () => {
  ...
  // Giả lập xử lý upload file
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      // Trong thực tế sẽ gọi API upload file ở đây
      setValue("businessLicenseUrl", "https://res.cloudinary.com/jobfy/image/upload/v1/licenses/sample.pdf");
  ...
  const isFullyVerified = progress?.step1.isCompleted && progress?.step2.isCompleted && progress?.step3.isVerified;
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[12px] font-black text-[#64748B] uppercase tracking-wider">
            HOÀN THÀNH {stepsCompleted}/3 BƯỚC
          </span>
        </div>
        ...
        <div className={`bg-white rounded-3xl p-6 border ${progress?.step3.isVerified ? 'border-emerald-100' : 'border-[#E2E8F0]'} space-y-6 shadow-sm`}>
          <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
            ...
            <div className="flex-1 space-y-1">
              <div className="flex items-center gap-2">
                ...
                <span className={`px-2 py-0.5 text-[10px] font-black rounded-md border uppercase ${
                  progress?.step3.isVerified ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 
          ...
          {!progress?.step3.isVerified && (
            ...
            <div className="flex flex-col gap-2">
              ...
              <label className={`flex-1 min-h-[140px] border-2 border-dashed rounded-xl flex flex-col items-center justify-center gap-2 transition-all ${isStep3Pending ? 'bg-slate-50 border-slate-200' : watch("businessLicenseUrl") ? 'bg-emerald-50 border-emerald-200' : 'bg-[#F8FAFC] border-[#CBD5E1] hover:bg-[#F1F5F9] cursor-pointer group'}`}>
                <input type="file" className="hidden" accept=".pdf,.jpg,.jpeg,.png" onChange={handleFileUpload} disabled={isStep3Pending} />
                <div className="w-10 h-10 rounded-full bg-[#E2E8F0] flex items-center justify-center text-[#00307c]">
                  {watch("businessLicenseUrl") ? <CheckCircle2 className="text-emerald-600" size={20} /> : <Upload size={20} />}
                </div>
                <div className="text-center">
                  <p className="text-[14px] font-black text-[#0F172A]">
                    {watch("businessLicenseUrl") ? "Đã chọn tệp thành công" : "Click để tải tệp lên"}
                  </p>
