...
import { authApi } from "@/features/auth/types/authApi";
import { UserProfile, EmployerLoginInput as LoginInput } from "./auth.types";
...
export const initAuth = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      // Sửa lỗi bóc tách: Dữ liệu nằm trong response.data.data
...
export const loginEmployer = createAsyncThunk(
  ...
  async (data: any, { rejectWithValue }) => {
    try {
      const res = await authApi.loginEmployer(data);
      // res.data là body { status, data: { accessToken, user } }
...
export const registerEmployer = createAsyncThunk(
  ...
  async (data: any, { rejectWithValue }) => {
    try {
      ...
      // register thường trả về body trực tiếp
      return res.data;
    } catch (err: any) {
