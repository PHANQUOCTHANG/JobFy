...
export function MarqueeStrip({ companies }: { companies: Company[] }) {
  ...
  return (
    <div className="bg-[#D44E2B] py-3.5 overflow-hidden flex gap-12">
      {[0, 1, 2].map((set) => (
        <div key={set} className="flex gap-12 shrink-0" style={{ animation: "marquee 22s linear infinite" }}>
          {companies.map((c, i) => (
            <span key={`${set}-${i}`} className="text-white/80 text-[13px] font-semibold whitespace-nowrap flex items-center gap-2 flex-shrink-0">
