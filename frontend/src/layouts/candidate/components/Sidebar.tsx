...
import { CANDIDATE_PATHS } from "@/config/paths";
...
const sidebarGroups = [
  {
    ...
    items: [
      ...
      {
        label: "Tìm việc làm",
        path: `/jobs`, // This goes to public job search page
        icon: Search,
      },
    ],
  },
  {
    ...
    items: [
      {
        label: "Hồ sơ & CV",
        path: `${CANDIDATE_PATHS.DASHBOARD}/${CANDIDATE_PATHS.PROFILE}`,
        icon: User,
      },
    ],
  },
  ...
  },
  {
    ...
    items: [
      {
        label: "Job Alerts",
        path: `${CANDIDATE_PATHS.DASHBOARD}/${CANDIDATE_PATHS.JOB_ALERTS}`,
        icon: BellRing,
      },
    ],
  },
          ...
          {sidebarGroups.map((group) => (
            <div key={group.title} className="flex flex-col gap-0.5">
              ...
              {group.items.map((item) => {
                const isActive =
                  item.path === CANDIDATE_PATHS.DASHBOARD
